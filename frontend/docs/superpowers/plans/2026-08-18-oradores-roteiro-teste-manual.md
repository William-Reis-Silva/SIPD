...
- [ ok ] **1.** Faça login como Administrador Global.
- [ ok ] **2.** Confirme que uma nova aba "Oradores" aparece na barra de abas.
- [ok ] **3.** Toque na aba "Oradores" → confirme que abre em `/oradores` com a
      lista vazia (ou com os oradores já cadastrados, se houver).
- [ok ] **4.** Toque em "Novo Orador" → confirme que navega para `/oradores/novo`
      permanecendo na aba Oradores (sem "piscar" para outra aba na barra).

## Bloco 2 — Cadastro, caminho feliz

- [ok ] **5.** Em `/oradores/novo`, preencha:
      - Nome
      - Sobrenome
...
- [ ok ] **10.** Abra o orador criado no Bloco 2, toque em "Editar" → altere o
      nome e o telefone → "Salvar" → confirme que os novos dados aparecem na
      tela de detalhe.
- [ok ] **11.** Ainda como Administrador Global (ou como Coordenador/Editor, se
...
- [ok ] **12.** Com o orador já vinculado a uma conta de usuário, faça login como
      ...
      origem."
- [ok ] **13.** Faça login como Administrador Global e tente a mesma alteração →
      confirme que é permitida (AG sempre pode alterar, independentemente do
      vínculo).

## Bloco 5 — Temas Preparados

- [ ok ] **14.** Na tela de detalhe do orador, mude para a aba "Temas
      Preparados" → "Adicionar Tema" → escolha um tema → confirme que ele
      aparece na lista.
- [ok ] **15.** Edite o campo "Observações" desse tema → "Salvar observações" →
      recarregue a tela e confirme que a observação persistiu.
- [ok ] **16.** Toque em "Remover" nesse tema → confirme que ele desaparece da
      lista de Temas Preparados.
- [ok ] **17.** Adicione o mesmo tema novamente e, em seguida, abra o seletor
...
- [ ok] **18.** Na tela de detalhe do orador, mude para a aba "Histórico" →
      ...
      antigo.
- [ok ] **19.** Faça login como Leitor (ou Editor/Coordenador) → abra o mesmo
...
- [ok ] **20.** Ainda como esse usuário não-Administrador-Global, confirme:
...
- [ok ] **21.** Em `/oradores`, digite parte de um nome na caixa de busca →
      confirme que a lista filtra corretamente.
- [ok ] **22.** Limpe a busca e digite parte de um telefone → confirme que a
      lista também filtra corretamente por telefone.
- [ ok] **23.** Selecione um tema específico em "Filtrar por tema" → confirme
      que só aparecem oradores que têm aquele tema como preparado.
- [ok ] **24.** Selecione "Todos os temas" novamente → confirme que a lista
      completa volta a aparecer.

## Bloco 8 — Console do navegador

- [ok ] **25.** Revise o console do navegador (aberto desde o início do
