-- ============================================================================
-- SIPD — Migração: catálogo completo de estados/cidades (IBGE)
-- ============================================================================
--
-- Contexto:
-- O conjunto básico (seed_estados_cidades_basico) tinha só 6 estados e 11
-- cidades — suficiente para testar o seletor Estado/Cidade da fatia
-- Congregações, mas não uma base real. Esta migração importa o catálogo
-- oficial completo do IBGE (27 UFs, 5571 municípios), obtido uma única
-- vez via API do IBGE (servicodados.ibge.gov.br/api/v1/localidades) e
-- gravado aqui como seed estático — sem dependência de chamadas externas em
-- runtime. Ver docs/superpowers memory "location data gap" para o histórico
-- do porquê a base estava incompleta.
--
-- Upsert por natureza: estados por UF (unique), cidades por (estado_id, nome)
-- (unique) — seguro rodar em ambiente que já tem o seed básico anterior.
-- ============================================================================

with estados_upsert as (
